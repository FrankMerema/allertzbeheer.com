    ...
    @if (centered()) {
      <section class="mx-auto mt-24 max-w-[1200px] px-6 pb-16 text-center md:px-8">
        <div class="rounded-xl border border-outline-variant bg-white px-8 py-16 shadow-sm">
          <h2 class="mb-4 font-headline-lg text-headline-lg text-on-surface">{{ title() }}</h2>
          <p class="mx-auto mb-10 max-w-xl font-body-md text-on-surface-variant">
            {{ description() }}
          </p>
      ...
      <section class="mx-auto max-w-[1200px] px-6 py-24 md:px-8">
        ...
        >
          <div class="max-w-2xl">
            <h2 class="mb-4 font-headline-lg text-on-surface">{{ title() }}</h2>
            <p class="font-body-lg text-on-surface-variant">{{ description() }}</p>
          </div>
          <div class="flex w-full flex-col gap-4 md:w-auto">
            <a
              [routerLink]="primaryRoute()"
              class="inline-flex items-center justify-center rounded-lg bg-primary px-10 py-5 font-headline-md text-on-primary transition-colors hover:bg-primary-container"
            >
              {{ primaryLabel() }}
            </a>
            @if (note()) {
